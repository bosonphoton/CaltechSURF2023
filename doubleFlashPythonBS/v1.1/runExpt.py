...
fileList = os.listdir("C:\\Users\\achan2\\Box\\SURF2023\\doubleFlashPythonBS\\v1.1\\Data") #checks inside the data
...
if str(task) == "vf":
    ...
    cop = copy.deepcopy(Data["Conditions"])
    temp = sorted(cop, key=lambda x: x[1]) #sorts arr based on 2nd element, which indicated catch vs normal trials \
    Data["Catch"] = [row[-2:] for row in temp[0:10]] #take only the catch trials rows, and only the Resp & RT of each
    
    
    for j in range(2): #for each location
        count = 0
        count0 = 0
        count1 = 0
        count2 = 0
        count3 = 0
        for i in range(10, len(temp)): #for the rest... "Present" trials
