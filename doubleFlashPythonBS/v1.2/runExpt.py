...
BS = bsExtraction(initials)

# initial parameters
env,param,stim,circLoc,win,deg = params(BS)
...
fileList = os.listdir("C:\\Users\\achan2\\Box\\SURF2023\\doubleFlashPythonBS\\v1.2\\Data") #checks inside the data
...
if foundFiles: #check if file exists
    ...
    Data["Complete"] = 0
    Data["leftcenterBS"] = BS['leftBS_center']
    Data["leftwidthBS"] = BS['leftBS_x'] * 2
    Data["leftheightBS"] = BS['leftBS_y'] * 2
    Data["rightcenterBS"] = BS['rightBS_center']
    Data["rightwidthBS"] = BS['rightBS_x'] * 2
    Data["rightheightBS"] = BS['rightBS_y'] * 2
...
if str(task) == "vf":
    ...
    Data["Catch"] = [row[-2:] for row in temp[0:n_catch]] #take only the catch trials rows, and only the Resp & RT of each
    
    
    for j in range(6): #for each location
        count = 0
        count0 = 0
        count1 = 0
        count2 = 0
        count3 = 0
        for i in range(n_catch, len(temp)): #for the rest... "Present" trials
    ...
    for j in range(6): #for each location
        ...
        count3 = 0
        for i in range(n_catch, len(temp)): #for the rest... "Present" trials
            if temp[i][0] == j and temp[i][2] == 1 and temp[i][3] == 2 and temp[i][4] <= 3:
                count += 1
                if temp[i][4] == 0:
                    count0 += 1
                elif temp[i][4] == 1:
    ...
    #cleanup(Data,filePath,el_tracker,win)
