...
# initial parameters
env,param,stim,circLoc,win,deg,sequence = params()
...
trials = genTrials(task)

fileList = os.listdir("C:\\Users\\achan2\\Box\\SURF2023\\rabbit\\Data") #checks inside the data
...
if str(task) =='av': #224 total trials, 64 catch
    Data["Catch"] = [row[-2:] for row in temp[0:64]] #take only the catch trials rows, and only the Resp & RT of each
...
if str(task) =='dt': #216 total trials, 32 catch
    Data["Catch"] = [row[-2:] for row in temp[0:32]] #take only the catch trials rows, and only the Resp & RT of each
