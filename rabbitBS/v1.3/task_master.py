...
from get_input_location import get_input_location
...
def task_master(stimshape,trials,task,env,param,stim,circLoc,win,deg,Data,filePath,eye,el_tracker,sequence):
    ...
    for n in range(len(Data["Responses"]),len(trials)):
        ...
        while not responded:
            ...
            if n % 30 == 0 and n != 0:  # break every 20 trials
                leftover = len(trials) - n
                l0 = "Always Fixate at Center\n\n"
                l1 = 'Please take a break if needed\n'
                l2 = '\n Press any key to continue\n'
                l3 = f'\n{leftover} more trials to go'
                drawStim(l0 + l1 + l2 + l3, win, stim, env)
                win.flip()
                playAudio('break', env)
            ...
            if str(task) == 'av':
                if num == 2 or num == 3:                                    
                    loc = get_input_location(num,win,stim,env)           
                    Data["Responses_Location"].append(loc)   
